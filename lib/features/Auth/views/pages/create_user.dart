...
import 'package:paper_safe_v2/features/Auth/viewmodel/auth_viewmodel.dart';
...
class _UserInfoState extends ConsumerState<UserInfo> {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(authViewModelProvider, (_, next) {
      next?.when(data: (data) {
        CommonWidgets().getSnackbar(context, "User Registered Successfully");
        Navigator.of(context)
            .pushReplacement(MaterialPageRoute(builder: (context) {
          return Homepage();
        }));
      }, error: (error, st) {
        CommonWidgets().getSnackbar(context, error.toString());
      }, loading: () {
        CommonWidgets().getSnackbar(context, "Loading");
      });
    });
    ...
    return Scaffold(
      ...
      body: isLoading
          ? CommonWidgets().getLoadingWidget()
          : Container(
              width: width,
